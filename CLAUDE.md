...
- Backend : Supabase, projet existant `mrqhesvdazroyqqcwprh` (organisation de l'utilisateur, partagé avec une autre app à lui ; un projet dédié coûterait 10 $/mois). Une seule table, `relief_projects` (id, owner, name, data jsonb, thumbnail, format, share_slug, dates), avec RLS : le propriétaire lit, écrit, supprime ; tout le monde lit une ligne dont `share_slug` est posé. `lib/cloud.ts` porte le client (URL et clé publique en dur, surchargeables par `NEXT_PUBLIC_SUPABASE_URL` / `_ANON_KEY`), le store `useCloud` (utilisateur, état de sauvegarde) et les opérations. Connexion par Google (à activer dans le tableau de bord Supabase, Authentication → Providers, avec un client OAuth Google) ou par lien magique e-mail ; l'URL du site et l'URL Vercel doivent être dans Authentication → URL configuration. `project.cloudId` est la ligne du projet ; une fois posée, `useCloudAutosave` réécrit le projet quatre secondes après chaque changement. La vignette est un rendu 480 px en data URL dans la ligne. Un lien `/s/<slug>` charge la scène en copie (`cloudId` nul) et redirige sur l'éditeur. Les fichiers importés (modèles, médias, polices) restent dans l'IndexedDB de la machine : un projet cloud transporte la scène, pas les octets.
