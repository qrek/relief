...
import { hasKeys, keyStateOf, sampleParams } from "../../lib/keyframes";
...
function EffectCard({
  ...
  const shown = hasKeys(instance.keys) ? sampleParams(instance.keys, tick.time, instance.params) : instance.params;
        ...
        <div className="flex flex-col gap-2">
          {def.params.map((p) => (
            <Slider
              ...
              keyState={keyStateOf(instance.keys, [p.key], tick.time)}
              onKey={() => toggleKeys({ kind: "effect", ownerId, instanceId: instance.id, channels: [p.key] }, sceneClock.clipTime)}
              min={p.min}
              max={p.max}
              step={p.step}
